...
- 文档导航，按用途索引使用指南、API、版本记录和维护入口；
- 18 项启动同步与 Adapter 模式读取回归测试。

### Changed

- 启动桥接时自动同步启用账户的 Adapter 模式，保留账户配置与 Profile；新版 Adapter 在命令轮询前读取模式变化，无需逐次手工修改配置或重启策略；
- 模式同步前校验账户绑定及交易 Profile，多账户预检或写入失败时停止启动，熔断状态只同步观察模式。
- Release 解压目录重新分层：内部安装文件归入 `installer`，详细资料归入 `docs`；
- 桌面只生成启动和状态两个脚本；
- `查看QMT桥接状态.cmd` 会先执行完整连接验证，再显示当前状态；
- 不再生成独立的验证和打开 QMT 配置目录脚本，升级时安全清理本项目生成的旧入口。

### Fixed

- 修正发布包使用说明中错位的第 5 步并补齐文档链接；
- 修复中文安装入口在部分 Windows 代码页下闪退；
- 修复源码 editable 安装路径末尾反斜杠导致 pip 收到多余引号。
